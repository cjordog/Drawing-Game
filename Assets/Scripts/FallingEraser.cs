...
public class FallingEraser : MonoBehaviour {
    ...
    {
        RaycastHit2D enemyHit = Physics2D.Raycast(this.transform.position + Vector3.down*.175f, direction, rayLength, obstacleMask);
        //Debug.DrawRay(this.transform.position - direction * .175f, direction * rayLength, Color.red);
        if (enemyHit)
        {
